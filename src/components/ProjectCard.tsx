...
export default function ProjectCard({ titulo, categoria, problema, solucion, aporte, stack, repoUrl, repoLabel = "Código", demoUrl, estado, codigo, tono, destacado = false, number, image, imageAlt, imagePosition = "center" }: ProjectCardProps) {
  return <article className={`project-card ${destacado ? "project-card-featured" : ""}`}>
    <div className={`project-visual project-visual-${tono}`} aria-label={image ? undefined : `Portada gráfica original de ${titulo}`} role={image ? undefined : "img"}>{image ? <Image className="project-image" src={image} alt={imageAlt ?? `Captura real del producto ${titulo}`} fill sizes={destacado ? "(max-width: 760px) 100vw, 42vw" : "(max-width: 760px) 100vw, 50vw"} style={{ objectPosition: imagePosition }} /> : codigo === "NV" ? <div className="nexovet-preview" aria-hidden="true"><div className="nv-top"><b>N</b><span>NexoVet</span><i /></div><div className="nv-body"><aside><i /><i /><i /><i /></aside><main><span>Dashboard</span><div className="nv-metrics"><i /><i /><i /></div><div className="nv-panel"><i /><i /><i /><i /><i /></div></main></div></div> : <><span className="project-code">{codigo}</span><div className="project-ui" aria-hidden="true"><i /><i /><i /></div></>}<div className="project-overlay" aria-hidden="true" /><span className="visual-number">{number}</span><span className="visual-label">{image ? "Producto real" : codigo === "NV" ? "Sistema integral" : "Portada conceptual"}</span></div>
